...
def now_db() -> datetime.datetime:
    """Lokale Zeit, auf Sekunden gerundet – passt zu MariaDB DATETIME ohne Mikrosekunden."""
...
def load_config(ctx=None, command_name: str = "") -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    cfg.update({
        "HEALTH_WINDOW_MIN": _env_int("HEALTH_WINDOW_MIN", 120),
        "HEALTH_AV_FINDINGS_MAX": _env_int("HEALTH_AV_FINDINGS_MAX", 0),
        "HEALTH_FLAGGED_MAX": _env_int("HEALTH_FLAGGED_MAX", 0),
        "HEALTH_SAFEAGE_MIN": _env_int("HEALTH_SAFEAGE_MIN", 30),
    })
...
def write_scan_summary(conn,
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            # Insert run_summaries (für status-Befehl)
            # Bestimme cmd anhand von note oder scan_paths
            cmd = "scan"
            if note and "av-scan" in note:
                cmd = "av-scan"
            elif scan_paths and any(x in (scan_paths or "") for x in ["av-scan", "clamav"]):
                cmd = "av-scan"
                
            cur.execute("""
                INSERT INTO run_summaries
                  (source, cmd, started_at, finished_at, candidates, files_processed, bytes_processed,
                   flagged_new, changed_count, reverified_count, scan_paths)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            """, (
                src, cmd, started_at, finished_at,
                int(candidates_count), int(files_processed), int(bytes_processed),
                int(flagged_new_count), int(changed_count), int(reverified_count),
                (scan_paths or None)
            ))
...
## Beispiele:
  ...
  # 4) ClamAV (Virenscanner)
      ...
      python entropywatcher.py av-scan --paths "/srv/nas/User1,/srv/nas/User2"
  ...
  # 5) Einzelne Datei vom Scoring ausnehmen / wieder normal
      ...
      python entropywatcher.py tag-exempt "/srv/nas/User1/foo.bin"
      python entropywatcher.py tag-normal "/srv/nas/User1/foo.bin"
...
@click.pass_context
def db_report(ctx, what, source, hours, limit):
    """
    ...
    """
    cfg, trace = load_config(ctx, "db-report")
    logger, q, listener = setup_logging(cfg)
...
@click.option("--debug", is_flag=True, help="Debug: Schwellen + rohe SAFEAGE anzeigen.")
@click.pass_context
def status_cmd(ctx, window_min, json_out, debug):
    """
    ...
    """
    cfg, trace = load_config(ctx, "status")
    logger, q, listener = setup_logging(cfg)
    try:
        ...
        # ENV/Defaults (aus cfg, nicht os.getenv!)
        W = int(cfg.get("HEALTH_WINDOW_MIN", 120))
        if window_min is not None: W = max(5, window_min)
        AV_MAX = int(cfg.get("HEALTH_AV_FINDINGS_MAX", 0))
        FLAG_MAX = int(cfg.get("HEALTH_FLAGGED_MAX", 0))
        SAFEAGE = int(cfg.get("HEALTH_SAFEAGE_MIN", 30))
        ...
        # Defensive: SOURCE_LABEL für nachfolgende Queries (falls versehentlich entfernt bei Merge/Patch)
        source = cfg.get("SOURCE_LABEL") or None

        # 1) Letzte Läufe aus run_summaries (gefiltert nach source)
        if source:
            cur.execute("""
                SELECT cmd, MAX(finished_at) AS last_ts
                FROM run_summaries
                WHERE source = %s AND finished_at >= %s
                GROUP BY cmd
            """, (source, t_from))
        else:
            cur.execute("""
                ...
                GROUP BY cmd
            """, (t_from,))
        runs = {r["cmd"]: r["last_ts"] for r in cur.fetchall()}
        ...
        if source:
            cur.execute("""
                ...
                WHERE flagged=1 AND (last_time >= %s OR missing_since >= %s) AND source=%s
            """, (t_from, t_from, source))
        else:
            cur.execute("""
                SELECT COUNT(*) AS c FROM files
        ...
        # Prüfe ob ClamAV für diesen Service aktiv ist
        clamav_enabled = cfg.get("CLAMAV_ENABLE", False)
        
        # 4) Heuristik bewerten (Service-spezifisch!)
        age_ok = True
        relevant_ts = last_av_ts if clamav_enabled else last_scan_ts
        age_min = None
        if relevant_ts:
            age_min = (now - relevant_ts).total_seconds()/60.0
            age_ok = (age_min >= SAFEAGE)
        
        status = "green"; code = 0; reasons = []
        
        # Service-spezifisch prüfen: entweder scan ODER av-scan muss vorhanden sein
        if clamav_enabled:
            # Service führt nur AV-Scans durch -> nur av-scan prüfen
            if not last_av_ts:
                status, code = "yellow", 1
                reasons.append("no_recent_runs")
        else:
            # Service führt nur Entropy-Scans durch -> nur scan prüfen
            if not last_scan_ts:
                status, code = "yellow", 1
                reasons.append("no_recent_runs")
        if av_count > AV_MAX:
        ...
        if debug:
            report["thresholds"] = {
                "HEALTH_WINDOW_MIN": W,
                "HEALTH_AV_FINDINGS_MAX": AV_MAX,
                "HEALTH_FLAGGED_MAX": FLAG_MAX,
                "HEALTH_SAFEAGE_MIN": SAFEAGE,
                "clamav_enabled": bool(clamav_enabled),
                "relevant_ts": (relevant_ts.isoformat() if relevant_ts else None)
            }
